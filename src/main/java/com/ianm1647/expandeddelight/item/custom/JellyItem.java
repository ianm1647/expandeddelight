...
public class JellyItem extends ConsumableItem {
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        if (itemStack.isOf(ItemList.SWEET_BERRY_JELLY)) {
            tooltip.add(new TranslatableText("item.expandeddelight.sweet_berry_jelly.tooltip").formatted(Formatting.BLUE));
        }
        if (itemStack.isOf(ItemList.GLOW_BERRY_JELLY)) {
            tooltip.add(new TranslatableText("item.expandeddelight.glow_berry_jelly.tooltip").formatted(Formatting.BLUE));
        }
    }
}
